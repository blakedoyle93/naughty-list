...
import { isDiscordWebhook, buildMessage, postAlert } from './discord'
...
describe('buildMessage', () => {
  it('puts a siren either side of the headline, in the text Discord pushes', () => {
    ...
    expect(m.embeds[0].title).toBe('\u{1F6A8} Naughty gaymer alert \u{1F6A8}')
  })

  it('names the player, the note, their side and who wrote it', () => {
    const m = buildMessage(game('ChampSelect'), [hit('fockoff', 'enemy', 'dodges every game')])
    expect(m.embeds[0].description).toBe(
      '**fockoff#OCE** - dodges every game _(against us)_ \u2014 Blake'
    )
    expect(m.embeds[0].footer.text).toBe('Spotted in champ select')
  })

  it('lists everyone when there is more than one', () => {
    const m = buildMessage(game('InProgress'), [
      hit('fockoff', 'enemy', 'dodges'),
      hit('jared', 'ally', 'int feeds')
    ])
    expect(m.content).toContain('fockoff, jared')
    expect(m.embeds[0].description).toContain('**jared#OCE** - int feeds _(on our team)_')
    expect(m.embeds[0].footer.text).toBe('Spotted in the game')
  })
